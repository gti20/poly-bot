...
class PolymarketClient:
    ...
    def place_limit_order(
        ...
        """Place a GTC limit order - compatible with your current py_clob_client version."""
        if self.settings.dry_run:
            logger.info(
                "[DRY-RUN] %s %s @ %.4f  size=%.2f USDC",
        ...
        try:
            ...
            # This style works on older client versions (what your other bot uses)
            signed_order = client.create_order(order)
            resp = client.post_order(signed_order)   # defaults to GTC

            logger.info("✅ ORDER PLACED: %s %s @ %.4f", side, token_id[:12]+"...", price)
            return resp
    ...
    def cancel_and_sell_position(
        ...
        if self.settings.dry_run:
            logger.info(
                "[DRY-RUN] Would sell %.4f of token %s @ %.4f",
                size, token_id[:12]+"...", current_bid
            )
            return {"dry_run": True}
